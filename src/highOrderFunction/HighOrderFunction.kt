...
//        println(minIntResult)
        ...
        printLastStr("Unc", 2) { str ->
            str.last()
        }

        printLastStr("hoge", 2) { str ->
            str.last()
        }

    }

    private fun printLastStr(string: String, i: Int, getLast: (String) -> Char) {
        println(getLast(string))
    }
    ...
    private fun <T>min(collection: Collection<T>, more: (T, T) -> Boolean): T? {
        ...
        for (it in collection) {
            ...
            if (min == null || more(min, it)) { // a.lgt > b
                min = it
            }
        }
        return min
